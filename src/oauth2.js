...
server.grant(oauth2orize.grant.code((client, redirectUri, user, ares, done) => {
  ...
  console.log('Granting authorization codes::: ' + code);
...
server.grant(oauth2orize.grant.token((client, user, ares, done) => {
  ...
  console.log('Granting implicit authorization: ' + token);
...
server.exchange(oauth2orize.exchange.code((client, code, redirectUri, done) => {
  console.log('Exchanging authorization codes....');
  db.authorizationCodes.find(code, (error, authCode) => {
    if (error) return done(error);
    console.log(authCode);//TODO Manuel to review this
...
server.exchange(oauth2orize.exchange.password((client, username, password, scope, done) => {
  console.log('Exchanging user id and password for access tokens');
  // Validate the client
...
console.log('Exchanging the client id and password/secret for an access token');
  // Validate the client
  db.clients.findByClientId(client.clientId, (error, localClient) => {
    if (error) return done(error);
    if (!localClient) return done(null, false);
